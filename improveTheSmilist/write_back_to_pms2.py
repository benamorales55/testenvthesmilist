...
def coverage_table():
    ...
    dq_matituck_plan, uhccp_plan, uhccp_dual_plan,uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan,lb_mattituck_plan =  check_plan_conditions().values()

    if dq_matituck_plan or dq_fishkill_plan or dq_catskill_plan:
...
#     # --- Estado inactivo y termed ---
#     elif 'inactive' in status or 'termed' in status:
#         checkbox_not_elegible.update(False)
#         if Value_eligibility_end:
#             print(f"*********Value_eligibility_end: {Value_eligibility_end} is setting")
#             if eligibility_end.is_enabled():
#                 eligibility_end.update(Value_eligibility_end)
#                 log.append(f"elig_end → {Value_eligibility_end}")
#             dates_updated = True
...
#             log.append("chk_not_elg → True (inactive)")
#         else:
#             log.append("chk_not_elg → True (termed)")
...
#             dates_updated = True
#         else:
#             setLog("Dates were not updated because no effective date was found|")
#             winAction.closeModals("Dentrix Dental Systems|Insurance Information", duration=1)
#             return 1
...
#     if 'inactive' not in status and 'active' in status:
#         openWin("famFile > insInfo")
#         ins_info.scope()
#         date_error = False
#         if Value_last_eligibility_check:
#             if parse_date(last_eligibility_check.get_text()) != parse_date(Value_last_eligibility_check):
#                 date_error = True
#         if Value_eligibility_start:
#             if parse_date(eligibility_start.get_text()) != parse_date(Value_eligibility_start):
#                 date_error = True
#         if Value_eligibility_end and Value_eligibility_start:
#             if parse_date(eligibility_end.get_text()) != parse_date(Value_eligibility_end):
#                 date_error = True
...
def verified_employer():
    ...
    clinic_found = any(data_supplies['practice'] in lista for lista in iv_config["elg_clinic"].values())
    dq_matituck_plan, uhccp_plan, uhccp_dual_plan,uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan,lb_mattituck_plan =  check_plan_conditions().values()
    if hmo_uhc:
    ...
    elif csea_all_plan and re.search(static_regex["csea_reg"],data_supplies['carrier_name'],re.IGNORECASE):
        ...
        if matchcsea:
            ...
            return False
    elif dq_fishkill_plan or dq_catskill_plan:
        ...
        elg_plan = any([dq_matituck_plan, uhccp_plan, uhccp_dual_plan, uhccp_nj_plan,cs_all_plan,uhccp_middleisl,lb_mattituck_plan])
...
#         # if option.lower() == "csea":
#         #     if 'CSEA' in data[clinic_name]:
#         #         nodo = data[clinic_name]['CSEA']['Plan Type']
#         #         nodo_plan = next(
#         #             (value for key,value in nodo.items() if dental_plan in key),
#         #             {}
#         #         )
#         #         if nodo_plan:
#         #             return nodo_plan
...
#                 plan_name_aliases = {
#                     "EBF MEMBER PLUS": "EBF",
#                     # Agregá más si surgen otros casos
#                 }

#                 # Normalización del nombre del plan
#                 normalized_plan = plan_name_aliases.get(dental_plan.upper(), dental_plan)
...
def get_elg_info(carrier_name: str, group_name: str):
    ...
    #     else:
    #         csea_data = fee_schedule_elg('ONN','csea')
...
def input_api_and_db(employer_name: str,nomenclature_plan = None):
    dq_matituck_plan, uhccp_plan, uhccp_dual_plan,uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan,lb_mattituck_plan =  check_plan_conditions().values()
    elg_plan = any([dq_matituck_plan, uhccp_plan, uhccp_dual_plan, uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan,lb_mattituck_plan])
...
def get_group_plan_dict():
    ...
    elif uhccp_plan or uhccp_dual_plan or uhccp_nj_plan or uhccp_middleisl:
...
def check_plan_conditions():
    ...
    # dq_catskill_plan = (is_elg_plan("Dentaquest") and 
    # bool(re.search(r'(denta quest|dentaquest)',data_supplies['carrier_name'],re.IGNORECASE))
    # and data_supplies['practice'].lower() == 'catskill')

    dq_catskill_plan = (bool(re.search(r'(denta quest|dentaquest)',data_supplies['carrier_name'],re.IGNORECASE))
    and data_supplies['practice'].lower() == 'catskill')

    lb_mattituck_plan = (bool(re.search(static_regex['liberty'],data_supplies['carrier_name'],re.IGNORECASE))
    and data_supplies['practice'].lower() == 'mattituck')
    ...
    elg_plans_flags = {
        ...
        'dq_catskill_plan' : dq_catskill_plan,
        'lb_mattituck_plan' : lb_mattituck_plan,
        }
        ...
        if is_ready_to_upload and is_coveraged:
            ...
            if not("HMO" in FeeScheduleName and re.compile('(?i)^Aetna.*|^Foreign.*').search(data_supplies['carrier_name'])):
                ...
                "MAX OUT" not in data_supplies["verification_status"].upper()):     
                    ...
                    print_log(SUCCESS,"ELG PLAN dq_matituck_plan, uhccp_plan, uhccp_dual_plan,uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan")
                    print(dq_matituck_plan, uhccp_plan, uhccp_dual_plan,uhccp_nj_plan,cs_all_plan,uhccp_middleisl,csea_all_plan,hmo_uhc,dq_fishkill_plan,dq_catskill_plan,lb_mattituck_plan)
                    ...
                    if elg_plan:
                        ...
                        if employer_:update_employer(employer_)  
                        coverage_table()
                        if uhccp_plan or uhccp_dual_plan or uhccp_nj_plan:
                            max_value = '9999'
